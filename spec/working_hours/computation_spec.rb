...
describe WorkingHours::Computation do
  ...
  describe '#add_days' do
    ...
    it 'can add working days to time' do
      time = Time.local(1991, 11, 15, 14, 0o0, 42)
      expect(add_days(time, 1)).to eq(Time.local(1991, 11, 18, 14, 0o0, 42)) # Monday
    end

    it 'can add working days to ActiveSupport::TimeWithZone' do
      time = Time.utc(1991, 11, 15, 14, 0o0, 42)
      time_monday = Time.utc(1991, 11, 18, 14, 0o0, 42)
    ...
    it 'skips non worked days' do
      ...
      WorkingHours::Config.new.working_hours = { mon: { '09:00' => '17:00' }, wed: { '09:00' => '17:00' } }
    ...
    it 'skips non worked days when origin is not worked' do
      ...
      WorkingHours::Config.new.working_hours = { mon: { '09:00' => '17:00' }, wed: { '09:00' => '17:00' }, thu: { '09:00' => '17:00' }, sun: { '09:00' => '17:00' } }
    ...
    it 'skips holidays' do
      time = Date.new(2014, 4, 7) # Monday
      WorkingHours::Config.new.holidays = [Date.new(2014, 4, 8)] # Tuesday
      expect(add_days(time, 1)).to eq(Date.new(2014, 4, 9)) # Wednesday
    end

    it 'skips holidays when origin is holiday' do
      time = Date.new(2014, 4, 9) # Wednesday
      WorkingHours::Config.new.holidays = [time] # Wednesday
    ...
    it 'skips holidays and non worked days' do
      time = Date.new(2014, 4, 7) # Monday
      WorkingHours::Config.new.holidays = [Date.new(2014, 4, 9)] # Wednesday
      WorkingHours::Config.new.working_hours = { mon: { '09:00' => '17:00' }, wed: { '09:00' => '17:00' } }
    ...
    it 'returns the original value when adding 0 days' do
      ...
      WorkingHours::Config.new.holidays = [time]
    ...
    it 'accepts time given from any time zone' do
      ...
      WorkingHours::Config.new.time_zone = 'Tokyo' # But we are at tokyo, so it's already Friday 6 am
  ...
  describe '#add_hours' do
    it 'adds working hours' do
      time = Time.utc(1991, 11, 15, 14, 0o0, 42) # Friday
      expect(add_hours(time, 2)).to eq(Time.utc(1991, 11, 15, 16, 0o0, 42))
    end

    it 'can substract working hours' do
      time = Time.utc(1991, 11, 18, 14, 0o0, 42) # Monday
      expect(add_hours(time, -7)).to eq(Time.utc(1991, 11, 15, 15, 0o0, 42)) # Friday
    end

    it 'accepts time given from any time zone' do
      time = Time.utc(1991, 11, 15, 7, 0, 0) # Friday 7 am UTC
      WorkingHours::Config.new.time_zone = 'Tokyo' # But we are at tokyo, so it's already 4 pm
    ...
    it 'moves correctly with multiple timespans' do
      WorkingHours::Config.new.working_hours = { mon: { '07:00' => '12:00', '13:00' => '18:00' } }
  ...
  describe '#add_seconds' do
    ...
    it 'calls precompiled only once' do
      ...
      expect_any_instance_of(WorkingHours::Config).to receive(:precompiled).once.and_return(precompiled) # in_config_zone and add_seconds
      time = Time.utc(1991, 11, 15, 16, 59, 42) # Friday
      add_seconds(time, 120)
    end

    it 'supports midnight when advancing time' do
      WorkingHours::Config.new.working_hours = { mon: { '00:00' => '24:00' } }
    ...
    it 'supports midnight when rewinding time' do
      WorkingHours::Config.new.working_hours = { mon: { '00:00' => '24:00' }, tue: { '08:00' => '18:00' } }
      time = Time.utc(2014, 4, 8, 0, 0, 30) # Tuesday
      expect(add_seconds(time, -60)).to eq(Time.utc(2014, 4, 7, 23, 59, 0o0))
    end

    context 'with holiday hours' do
      before do
        WorkingHours::Config.new.working_hours = { thu: { '08:00' => '18:00' }, fri: { '08:00' => '18:00' } }
      end

      context 'with a later starting hour' do
        before do
          WorkingHours::Config.new.holiday_hours = { Date.new(2019, 12, 27) => { '10:00' => '18:00' } }
        end
      ...
      context 'with an earlier ending hour' do
        before do
          WorkingHours::Config.new.holiday_hours = { Date.new(2019, 12, 27) => { '08:00' => '17:00' } }
        end
      ...
      context 'with an earlier starting time in the second set of hours within a day' do
        before do
          WorkingHours::Config.new.working_hours = { thu: { '08:00' => '18:00' }, fri: { '08:00' => '12:00', '13:00' => '18:00' } }
          WorkingHours::Config.new.holiday_hours = { Date.new(2019, 12, 27) => { '08:00' => '12:00', '14:00' => '18:00' } }
        end
  ...
  describe '#advance_to_working_time' do
    it 'jumps non-working day' do
      WorkingHours::Config.new.holidays = [Date.new(2014, 5, 1)]
    ...
    it 'jumps outside holiday hours' do
      WorkingHours::Config.new.working_hours = { fri: { '08:00' => '18:00' } }
      WorkingHours::Config.new.holiday_hours = { Date.new(2019, 12, 27) => { '10:00' => '18:00' } }
    ...
    it 'do not leak nanoseconds when advancing' do
      expect(advance_to_working_time(Time.utc(2014, 4, 7, 5, 0, 0,
                                              123_456.789))).to eq(Time.utc(2014, 4, 7, 9, 0, 0, 0))
    end

    it 'returns correct hour during positive time shifts' do
      WorkingHours::Config.new.working_hours = { sun: { '09:00' => '17:00' } }
      ...
      expect(advance_to_working_time(Time.new(2020, 3, 29, 5, 0, 0,
                                              '+01:00'))).to eq(Time.new(2020, 3, 29, 9, 0, 0, '+02:00'))
      expect(advance_to_working_time(Time.new(2020, 3, 29, 1, 0, 0,
                                              '+02:00'))).to eq(Time.new(2020, 3, 29, 9, 0, 0, '+02:00'))
    end
    ...
    it 'returns correct hour during negative time shifts' do
      ...
      # starting from wrong time-zone
      expect(advance_to_working_time(Time.new(2020, 10, 25, 4, 0, 0,
                                              '+02:00'))).to eq(Time.new(2020, 10, 25, 9, 0, 0, '+01:00'))
      expect(advance_to_working_time(Time.new(2020, 10, 25, 1, 0, 0,
                                              '+01:00'))).to eq(Time.new(2020, 10, 25, 9, 0, 0, '+01:00'))
    end
  end
  ...
  describe '#advance_to_closing_time' do
    ...
    context 'with holiday hours' do
      ...
      it 'takes into account reduced holiday closing' do
        WorkingHours::Config.new.holiday_hours = { Date.new(2019, 12, 27) => { '10:00' => '17:00' } }
      ...
      it 'takes into account extended holiday closing' do
        WorkingHours::Config.new.holiday_hours = { Date.new(2019, 12, 26) => { '10:00' => '21:00' } }
        expect(advance_to_closing_time(Time.utc(2019, 12, 26, 20))).to eq(Time.utc(2019, 12, 26, 21))
      end
    end

    it 'do not leak nanoseconds when advancing' do
      expect(advance_to_closing_time(Time.utc(2014, 4, 7, 5, 0, 0,
                                              123_456.789))).to eq(Time.utc(2014, 4, 7, 17, 0, 0, 0))
    end
    ...
    it 'returns correct hour during positive time shifts' do
      ...
      expect(res.utc_offset).to eq(7200)
      # starting from wrong time-zone
      expect(advance_to_closing_time(Time.new(2020, 3, 29, 5, 0, 0,
                                              '+01:00'))).to eq(Time.new(2020, 3, 29, 17, 0, 0, '+02:00'))
      expect(advance_to_closing_time(Time.new(2020, 3, 29, 1, 0, 0,
                                              '+02:00'))).to eq(Time.new(2020, 3, 29, 17, 0, 0, '+02:00'))
    end
    ...
    it 'returns correct hour during negative time shifts' do
      ...
      expect(res.utc_offset).to eq(3600)
      # starting from wrong time-zone
      expect(advance_to_closing_time(Time.new(2020, 10, 25, 4, 0, 0,
                                              '+02:00'))).to eq(Time.new(2020, 10, 25, 17, 0, 0, '+01:00'))
      expect(advance_to_closing_time(Time.new(2020, 10, 25, 1, 0, 0,
                                              '+01:00'))).to eq(Time.new(2020, 10, 25, 17, 0, 0, '+01:00'))
    end
  end
  ...
  describe '#return_to_working_time' do
    ...
    it 'returns correct hour during positive time shifts' do
      WorkingHours::Config.new.working_hours = { sun: { '00:00' => '01:00' } }
      WorkingHours::Config.new.time_zone = 'Paris'
      ...
      expect(res.utc_offset).to eq(3600)
      # starting from wrong time-zone
      expect(return_to_working_time(Time.new(2020, 3, 29, 2, 0, 0,
                                             '+02:00'))).to eq(Time.new(2020, 3, 29, 1, 0, 0, '+01:00'))
      expect(return_to_working_time(Time.new(2020, 3, 29, 9, 0, 0,
                                             '+01:00'))).to eq(Time.new(2020, 3, 29, 1, 0, 0, '+01:00'))
    end
    ...
    it 'returns correct hour during negative time shifts' do
      ...
      expect(res.utc_offset).to eq(7200)
      # starting from wrong time-zone
      expect(return_to_working_time(Time.new(2020, 10, 25, 9, 0, 0,
                                             '+02:00'))).to eq(Time.new(2020, 10, 25, 1, 0, 0, '+02:00'))
      expect(return_to_working_time(Time.new(2020, 10, 25, 1, 0, 0,
                                             '+01:00'))).to eq(Time.new(2020, 10, 25, 1, 0, 0, '+02:00'))
    end
  end
  ...
  describe '#in_working_hours?' do
    ...
    context 'with holiday hours' do
      before do
        WorkingHours::Config.new.working_hours = { thu: { '08:00' => '18:00' }, fri: { '08:00' => '18:00' } }
        WorkingHours::Config.new.holiday_hours = { Date.new(2019, 12, 27) => { '10:00' => '20:00' } }
      end
  ...
  describe '#working_time_between' do
    ...
    it 'works across positive time shifts' do
      WorkingHours::Config.new.working_hours = { sun: { '08:00' => '21:00' } }
      WorkingHours::Config.new.time_zone = 'Paris'
      expect(working_time_between(
    ...
    it 'works across time shifts + midnight' do
      WorkingHours::Config.new.working_hours = { sun: { '00:00' => '24:00' } }
      WorkingHours::Config.new.time_zone = 'Paris'
      expect(working_time_between(
    ...
    it 'do not cause infinite loop if the time is not advancing properly' do
      ...
      end.to raise_error(RuntimeError, /Invalid loop detected in working_time_between \(from=2014-04-07T08:59:59.999/)
    end
    ...
    context 'with holiday hours' do
      before do
        WorkingHours::Config.new.working_hours = { mon: { '08:00' => '18:00' }, tue: { '08:00' => '18:00' } }
        WorkingHours::Config.new.holiday_hours = { Date.new(2014, 4, 7) => { '10:00' => '12:00', '14:00' => '17:00' } }
      end
