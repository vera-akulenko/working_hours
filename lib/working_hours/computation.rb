  ...
  module Computation
    ...
    def add_seconds(origin, seconds, config: nil)
      ...
      while seconds.positive?
        ...
        working_hours_for(time, config: config).each do |from, to|
          next unless (time_in_day >= from) && (time_in_day < to)
    ...
    def advance_to_working_time(time, config: nil)
      ...
      loop do
        ...
        time = (time + 1.day).beginning_of_day until working_day?(time, config: config)
        # find first working range after time
        time_in_day = time.seconds_since_midnight

        working_hours_for(time, config: config).each do |from, to|
          return time if (time_in_day >= from) && (time_in_day < to)
    ...
    def advance_to_closing_time(time, config: nil)
      ...
      loop do
        ...
        time_in_day = time.seconds_since_midnight
        working_hours_for(time, config: config).each_value do |to|
    ...
    def return_to_exact_working_time(time, config: nil)
      ...
      loop do
        # skip holidays and weekends
        time = (time - 1.day).end_of_day until working_day?(time, config: config)
    ...
    def in_working_hours?(time, config: nil)
      ...
      time_in_day = time.seconds_since_midnight
      working_hours_for(time, config: config).each do |from, to|
        return true if (time_in_day >= from) && (time_in_day < to)
      end
      false
    end
      ...
      if to < from
        ...
        while from < to
          ...
          working_hours_for(from, config: config).each do |begins, ends|
            if (time_in_day >= begins) && (time_in_day < ends)
          ...
          unless from > from_was
            raise "Invalid loop detected in working_time_between (from=#{from.iso8601(12)}, to=#{to.iso8601(12)}, distance=#{distance}, config=#{config}), please open an issue ;)"
          end
        end
    ...
    def wh_config
      WorkingHours::Config.new.precompiled
    end
    ...
    def in_config_zone(time, config: nil)
      if time.respond_to? :in_time_zone
        ...
        raise TypeError, "Can't convert #{time.class} to a Time"
      end
    end
