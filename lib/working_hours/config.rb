...
module WorkingHours
  ...
  class Config
    TIME_FORMAT = /\A([0-2][0-9]):([0-5][0-9])(?::([0-5][0-9]))?\z/.freeze
    ...
    def precompiled
      ...
      config[:precompiled] ||= begin
        ...
        compiled = { working_hours: Array.new(7) { {} }, holiday_hours: {} }
    ...
    def compile_time(time)
      ...
      return MIDNIGHT if time == 86_400
    ...
    def validate_hours!(dates)
      dates.each do |day, hours|
        ...
        hours.sort.each do |start, finish|
          ...
          elsif last_time && (start < last_time)
    ...
    def validate_working_hours!(week)
      raise InvalidConfiguration, :empty if week.empty?
    ...
    def validate_holidays!(holidays)
      unless holidays.respond_to?(:to_a)
        raise InvalidConfiguration.new :holidays_not_array, data: { holidays_class: holidays.class }
      end

      holidays.to_a.each do |day|
        raise InvalidConfiguration.new :holiday_not_date, data: { day: day } unless day.is_a? Date
      end
    end
    ...
    def validate_time_zone!(zone)
      if zone.is_a? String
        ...
        raise InvalidConfiguration.new :unknown_timezone, data: { zone: zone } if res.nil?
